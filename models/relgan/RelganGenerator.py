...
class Generator(Gen):
    ...
    def __init__(self, batch_size, seq_len, vocab_size, grad_clip, gpre_lr, **kwargs):
        ...
        self.x_fake = tf.placeholder(
            tf.int32, [batch_size, seq_len], name="x_fake")
        ...
        self.x_fake_onehot=tf.one_hot(self.x_fake, vocab_size)
        assert self.x_fake_onehot.get_shape().as_list() == [
            batch_size, seq_len, vocab_size]
    ...
    def init_generator(
        ...
        self.one_hot_value = tf.one_hot(tf.to_int32(tf.reshape(x_real, [-1])), vocab_size, 1.0, 0.0) * tf.reshape(
            g_predictions, [-1, vocab_size])
        self.ppl_slipt = self.one_hot_value[:, :vocab_size - 1]
        self.ppl_one = tf.reduce_sum(self.ppl_slipt)
    ...
    def get_ppl(self, sess, x):
        ppl_one=sess.run(
            self.ppl_one, feed_dict={self.x_real: x}
        )
        return ppl_one
