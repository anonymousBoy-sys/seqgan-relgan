...
from models.relgan.RelganDiscriminatorNew import DiscriminatorNew
...
class Relgan(Gan):
    ...
    def init_real_training(self, data_loc):
        ...
        self.seq_len = self.sequence_length
        ...
        discriminator_new = DiscriminatorNew(
            batch_size=self.batch_size, seq_len=self.seq_len, vocab_size=self.vocab_size,
            dis_emb_dim=self.dis_emb_dim, num_rep=self.num_rep, sn=self.sn, grad_clip=self.grad_clip,
            splited_steps=self.splited_steps
        )
        self.set_discriminator(discriminator, discriminator_new)
        ...
        # Get losses
        real_sig, fake_sig, po_neg, d_loss_new, log_pg, g_loss, d_loss = get_losses(
            generator, discriminator, discriminator_new, self.gan_type)
        ...
        discriminator_new.set_train_op(
            d_loss_new, self.optimizer_name, self.d_lr, global_step=self.global_step,
            nadv_steps=self.nadv_steps, decay=self.decay, po_neg=po_neg, real_sig=real_sig, fake_sig=fake_sig
        )
        ...
        fake_dataloader = RealDataLoader(
            batch_size=self.batch_size, seq_length=self.seq_len,
        )
        valid_dataloader = RealDataLoader(
            batch_size=self.batch_size, seq_length=self.seq_len,
        )
        test_dataloader = RealDataLoader(
            batch_size=self.batch_size, seq_length=self.seq_len,
        )

        self.set_data_loader(gen_loader=gen_dataloader,
                             dis_loader=None, dis_train_loader=None, dis_valid_loader=None, oracle_loader=None,fake_loader=fake_dataloader, valid_loader=valid_dataloader, test_loader=test_dataloader)
    ...
    def train_real(self, data_loc=None):
        ...
        if self.restore:
            ...
            for epoch in range(self.pre_new_dis_num):
                ...
                # valid dataset
                merge_summary_valid = self.sess.run(self.discriminator_new.merge_summary_valid,
                    feed_dict={self.generator.x_real: self.valid_data_loader.random_batch(),
                               self.generator.x_fake: self.fake_data_loader.random_batch()})
                #train dataset
                merge_summary_train, _ = self.sess.run([self.discriminator_new.merge_summary_train, self.discriminator_new.train_op],
                    feed_dict={self.generator.x_real: self.gen_data_loader.random_batch(),
                               self.generator.x_fake: self.fake_data_loader.random_batch()})
                #tensorboard
            ...
            self.total_distance_for_relgan()
            # save pre_train
            saver.save(self.sess, os.path.join(self.save_path, 'pre_train'))
        ...
        for epoch in range(self.pre_new_dis_num):
            print("evaluate epoch：" + str(epoch))
            # valid dataset
            merge_summary_valid_ad = self.sess.run(self.discriminator_new.merge_summary_valid_ad,
                feed_dict={self.generator.x_real: self.valid_data_loader.random_batch(),
                           self.generator.x_fake: self.fake_data_loader.random_batch()})
            # train dataset
            merge_summary_train_ad, _ = self.sess.run(
                [self.discriminator_new.merge_summary_train_ad, self.discriminator_new.train_op],
                feed_dict={self.generator.x_real: self.gen_data_loader.random_batch(),
                           self.generator.x_fake: self.fake_data_loader.random_batch()})
            # tensorboard
