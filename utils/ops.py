...
def mlp(input_, output_sizes, act_func=tf.nn.relu, use_bias=True):
    ...
    for layer_id in range(num_layers):
        net = linear(
            net, output_sizes[layer_id], use_bias=use_bias, scope='linear_{}'.format(layer_id))
...
def conv2d(input_, out_nums, k_h=2, k_w=1, d_h=2, d_w=1, stddev=None, sn=False, padding='SAME', scope=None):
    ...
    with tf.variable_scope(scope or "Conv2d"):
        ...
        if sn:
            W = spectral_norm(W)
        b = tf.get_variable(
            "Bias", shape=[out_nums], initializer=tf.zeros_initializer)
        conv = tf.nn.conv2d(input_, filter=W, strides=[
...
def self_attention(x, ch, sn=False):
    ...
    f = conv2d(x, ch // 8, k_h=1, d_h=1, sn=sn,
               scope='f_conv')  # [bs, h, w, c']
    g = conv2d(x, ch // 8, k_h=1, d_h=1, sn=sn,
...
def create_output_unit(output_size, vocab_size):
    ...
    Wo = tf.get_variable('Wo', shape=[
                         output_size, vocab_size], initializer=create_linear_initializer(output_size))
    bo = tf.get_variable(
        'bo', shape=[vocab_size], initializer=create_bias_initializer())
...
def add_gumbel_cond(o_t, next_token_onehot, eps=1e-10):
    ...
    print("shape of next_token_onehot: {}".format(
        next_token_onehot.get_shape().as_list()))

    gumbel = -tf.log(-tf.log(v + eps) + eps, name="gumbel")
    topgumbels = gumbel + tf.reduce_logsumexp(o_t, axis=-1, keep_dims=True)
    topgumbel = tf.reduce_sum(
        next_token_onehot * topgumbels, axis=-1, keep_dims=True)
...
def gradient_penalty(discriminator, x_real_onehot, x_fake_onehot_appr, config):
    ...
    alpha = tf.random_uniform(
        shape=[config['batch_size'], 1, 1], minval=0., maxval=1.)
...
def get_losses(generator, discriminator, discriminator_new, gan_type):

    d_out_real = discriminator.get_logits(x_onehot=generator.x_real_onehot)
    d_out_fake = discriminator.get_logits(x_onehot=generator.gen_x_onehot_adv)

    #discriminator_new loss
    d_out_fake_new = discriminator_new.get_logits(x_onehot=generator.x_fake_onehot)
    d_out_real_new = discriminator_new.get_logits(x_onehot=generator.x_real_onehot)
    d_loss_real_new = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(
        logits=d_out_real_new, labels=tf.ones_like(d_out_real_new)
    ))
    d_loss_fake_new = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(
        logits=d_out_fake_new, labels=tf.zeros_like(d_out_fake_new)
    ))
    d_loss_new = d_loss_real_new + d_loss_fake_new
    #discriminator_new mark
    real_sig = tf.sigmoid(d_out_real_new)
    fake_sig = tf.sigmoid(d_out_fake_new)
    po_neg = tf.reduce_mean(real_sig) - tf.reduce_mean(fake_sig)
    ...
    if gan_type == 'standard':  # the non-satuating GAN loss
        ...
        GP = gradient_penalty(discriminator.get_logits, generator.x_real_onehot,
                              generator.gen_x_onehot_adv, config)
        d_loss += GP
        ...
        raise NotImplementedError(
            "Divergence '%s' is not implemented" % gan_type)

    # [1], measures the log p_g(x)
    log_pg = tf.reduce_mean(tf.log(generator.gen_o + 1e-10))

    return real_sig, fake_sig, po_neg, d_loss_new, log_pg, g_loss, d_loss
